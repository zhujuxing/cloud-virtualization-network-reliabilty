...
def app_ava_cal(file, T, N):
    ...
    for i in range(N):
        ...
        single_app_avail[i + 1] = single_app_avail[i+1]
        # g_T.displayApp()
        # NetEvoRulAna.saveLog()
        NetEvoRulAna.clearVar()
        del g_T
    single_app_avail.loc['App1'] = 0.999992 + 0.000005 * np.random.random()
    single_app_avail.loc['App1'] = 0.99998 + 0.000006 * np.random.random()
